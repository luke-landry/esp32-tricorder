...
void app_ble_print_address(uint8_t *address_value, uint8_t address_type, const char *address_name){
...
void app_ble_print_conn_descriptor(struct ble_gap_conn_desc *conn_descriptor){

    ESP_LOGI(TAG_BLE, "Connection handle..........%d", conn_descriptor->conn_handle);

    app_ble_print_address(conn_descriptor->our_ota_addr.val, conn_descriptor->our_ota_addr.type, "Local OTA address");
    app_ble_print_address(conn_descriptor->our_id_addr.val, conn_descriptor->our_id_addr.type, "Local identity address");
    app_ble_print_address(conn_descriptor->peer_ota_addr.val, conn_descriptor->peer_ota_addr.type, "Peer OTA address");
    app_ble_print_address(conn_descriptor->peer_id_addr.val, conn_descriptor->peer_id_addr.type, "Peer identity address");
...
void app_ble_print_conn_update(const struct ble_gap_upd_params *update_parameters){
...
int app_ble_on_gap_event(struct ble_gap_event *event, void *arg){
    ...
    switch(event->type){
        ...
        case BLE_GAP_EVENT_CONN_UPDATE_REQ:
            ...
            app_ble_print_conn_update(event->conn_update_req.peer_params);
            break;
        default:
